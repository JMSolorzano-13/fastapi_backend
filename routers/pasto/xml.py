"""Pasto XML webhook — XML upload notifications from ADD.

Ported from: backend/chalicelib/blueprints/pasto/xml.py
1 route total.
"""
...
router = APIRouter(tags=["Pasto XML"])


@router.post("/" + envars.ADD_XML_WEBHOOK)
def xml_webhook(
    body: dict = Body(...),
    ...
    error, pasto_body, hdrs = parse_pasto_webhook(body, headers, "xml_webhook")
    ...
    with new_company_session_from_company_identifier(
        ...
        if not pasto_body:
            ...
            return {"status": "ok"}
        add_request.xmls_to_send_pending = pasto_body["ErrorRows"]
