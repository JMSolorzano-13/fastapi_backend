...
@bp.route(envars.ADD_CANCEL_WEBHOOK, methods=["POST"], cors=common.cors_config, read_only=False)
def cancel_webhook(session: Session):
    error, body, headers = bp_to_pasto_data(bp, "cancel_webhook")
    ...
    with new_company_session_from_company_identifier(
        ...
        if error or not body:  # Internal Pasto Error
            request.state = ADDSyncRequest.StateEnum.ERROR
            return {"status": "ok"}
        request.cfdis_to_cancel_pending = body["ErrorRows"]
        if body["ErrorRows"]:
            request.state = ADDSyncRequest.StateEnum.ERROR
