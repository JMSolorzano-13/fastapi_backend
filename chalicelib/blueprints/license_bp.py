...
from chalice import BadRequestError, NotFoundError, Response, UnauthorizedError
from sqlalchemy.orm import Session

from chalicelib.blueprints import common
from chalicelib.blueprints.product import (
...
def put(session: Session, user: User):
    json_body = bp.current_request.json_body or {}

    licenses = json_body.get("licenses", [])
...
@bp.route("/paid/alert", methods=["POST"], cors=common.cors_config)
def get_alert_invoice_paid():
    request = bp.current_request
    payload = request.raw_body.decode("utf-8")
    ...
    try:
        ...
        return Response(body={"error": "Invalid signature"}, status_code=400)
...
@bp.route("/source", methods=["PUT"], cors=common.cors_config, read_only=False)
def set_source(session: Session, user: User):
    json_body = bp.current_request.json_body or {}
...
@bp.route("/", methods=["POST"], cors=common.cors_config)
def get_license_details(session: Session, user: User):
    json_body = bp.current_request.json_body or {}
...
def set_license_by_user_v2(session: Session, user: User):
    json_body = bp.current_request.json_body or {}

    return _set_license_by_user_v2(session=session, json_body=json_body, user=user)
...
def _update_license(user, product_lines, proration_date):
    ...
    if status != "active":
        return new_subscription(user, product_lines)
    # Help in get correct price for conversion wih new price and renew price
...
@bp.route("/{workspace_identifier}", methods=["GET"], cors=common.cors_config)
def get_free_trial_by_workspace(workspace_identifier: str, session: Session):
    """Obtiene información de prueba gratuita de Siigo para un workspace."""
    owner_email = (
